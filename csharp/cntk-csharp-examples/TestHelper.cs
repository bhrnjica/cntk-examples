...
namespace CNTK.CSTrainingExamples
{
    ...
    public class TestHelper
    {
        ...
        public static float ValidateModelWithMinibatchSource(
            ...
            DeviceDescriptor device, int maxCount = 1000, List<List<float>> X =null, List<float> Y=null)
        {
            ...
            while (true)
            {
                ...
                //var scaledInput = CNTKLib.ElementTimes(imageInput,Constant.Scalar<float>(256f, device));
                var faetureData = minibatchData[featureStreamInfo].data.GetDenseData<float>(imageInput);
                ...
                if (totalCount > maxCount)
                {
                    ...
                    for (int i = 0; i < faetureData.Count && X!=null && Y!=null; i++)
                    {
                        var ii = faetureData[i].ToList();
                        X.Add(ii);
                        var aa = actualLabels[i];
                        Y.Add(aa);
